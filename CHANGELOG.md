...
- **Call recording format compatibility in Admin UI (branch `main`)**: Harmonized recording discovery and playback across call history and outbound APIs so audio files are resolved with case-insensitive suffix handling (`.wav`/`.WAV`, `.ulaw`/`.ULAW`). Call History recording playback now supports both WAV and uLaw captures: WAV files stream natively, while uLaw files are transcoded to browser-playable WAV on the fly. Outbound recording library lookups/preview paths were updated to use robust case-insensitive `.ulaw` resolution to avoid false "file not found" when filenames use uppercase extensions.
